...
class cluster_credit_model extends CI_Model {
...
function getConnector_Relationship_Officer($q,$id)
  ...
  function getRelationOfficer($id, $filter, $userType, $userType2, $date)
  {
      if($userType=='ADMIN')
  {  $Combine_Customers=array();	
     $this->db->select('*');
...
/*------------------------------------- Added by papiha on 02-05-2022---------------------------------------------------------------*/ 
 ...
 function get_IDCCR_bureauList($user_email)
           ...
           function Get_idccr_bureau_Filter($user_email,$searchValue)
        {
            ...
            if($searchValue!= '')
            {
            $where = "fname like '%".$searchValue."%' OR lname like '%".$searchValue."%' ";
            $this->db->where($where);
            }
            $this->db->order_by("id", "desc");
            $query = $this->db->get();
