                                ...
                                <div class="row">
                                    <div class="col-sm-12">
											...
											<div class="container">
												...
												<h3>Send Customer To FI Process</h3>
												</div>
												
												<br>
												</br>
												<Form action="<?php echo base_url('index.php/Credit_manager_user/do_upload_FI_doc');?>" method="post" enctype="multipart/form-data" id="FI_doc_upload">
												<div class="row">
												   <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
													<input type="number" name="form_id" value="3" hidden />
													<input type="text" name="send_to" value="FI" hidden />
												    ...
												    <div style=" margin-top: 8px;" class="col">
														 <label for="email" style="font-size: 16px; color: black; font-weight: bold;"> Select Document To upload</label>
															...
															<option value="">Select Documents</option>
																	<?php if (isset($FI_Doc_list)){ foreach($FI_Doc_list as $doc){ ?>
												...
												<?php  foreach($documents as $row){?>
													...
													<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
														<div class="row " style="border: 1px solid #edeff1; margin-left:13px; margin-right:13px">
															...
															<div class="col-sm-1"> 	
															<a style="margin-left: 8px; " href="<?php echo base_url();?>index.php/customer/delete_vendor_doc/<?php echo $row->id; ?>"><i class="fa fa-trash text-right" style="color:red;"></i></a>
												...
												<Form action="<?php echo base_url('index.php/Credit_manager_user/do_upload_FI_doc');?>" method="post" enctype="multipart/form-data">
												<div class="row">
												...
												<div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12">
													 <div style=" margin-top: 8px;" class="col">
														 <label for="email" style="font-size: 16px; color: black; font-weight: bold;"> Assign To FI</label>
													<?php if(!empty($Legal_docs)){if($Legal_docs->fi_status!='')
													{?>
													<select required class="form-control select" name="FI_name" id="FI_name" required> 
													<option value="<?php if(!empty($Legal_docs)){ echo $Legal_docs->fi_id; }?>" selected ><?php if(!empty($cust_info))echo $Legal_docs->fi_name;?></option>
													</select>
													  <input type="text" id="FI_name_1" name="FI_name_1"  value="<?php if(!empty($cust_info))echo $cust_info->Legal_name; ?>" hidden >
													<?php }else{?>
														<select required class="form-control" name="FI_name" id="FI_name" required > 
													<option value="" > select FI </option>
													
													</select>
													<?php }} else{?>
                                                      <input type="text" id="FI_name_1" name="FI_name_1" hidden >
													  <select required class="form-control" name="FI_name" id="FI_name" required > 
													<option value=""> select FI </option>
												...
												<?php if(!empty($Legal_docs)){if($Legal_docs->fi_status=='Revert From FI'){?>
													<div class="row">
														<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
														<div style=" margin-top: 8px;" class="col">
															 <label for="email" style="font-size: 16px; color: black; font-weight: bold;"> FI Comment </label>
															<textarea name="comment_FI" class="form-control"><?php echo $Legal_docs->fi_comment; ?> </textarea>
												...
												<?php if(!empty($Legal_docs)){if($Legal_docs->fi_status=='Document Received From FI')
													{ ?>
												<div class="text-center">
												<h3>FI Report</h3>
												...
												<?php if(isset($Legal_docs))
																   {
																	   ...
																	   $Conditions_to_Loan_Sanction_2= json_decode($Legal_docs->additional_comments);
																				   ...
																				   <div class="row">
																						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
																						    <div style=" margin-top: 8px;" class="col">
																							<label for="email" style="font-size: 16px; color: black; font-weight: bold;"> Comments From FI </label>
												...
												<?php }
												else if($Legal_docs->fi_status=='Document Received From FI')
														{?>
															<button type="submit" name="revert_to_FI" class="btn btn-primary" id='revert_to_FI' >Revert To FI</button>
														    <button type="submit" name="Accepted" class="btn btn-primary" id='Accepted' >Accept</button>
													<?php	}
													else if($Legal_docs->fi_status=='Send TO FI')
														{?>
                                                                <button type="submit" name="revert_action " class="btn btn-primary disabled" >Wating For FI Reports</button>
														<?php 
														}
														else if($Legal_docs->fi_status=='Revert Action Taken')
														{?>
                                                                <button type="submit" name="revert_action " class="btn btn-primary disabled" >Revert Action Taken</button>
														<?php
														}
														else if($Legal_docs->fi_status=='Revert From Finaleap')
														{?>
                                                                <button type="submit" name="revert_action " class="btn btn-primary disabled" >Revert From Finaleap</button>
														<?php
														}
														else if($Legal_docs->fi_status=='Accepted from Finaleap')
														{?>
                                                                <button type="submit" name="revert_action " class="btn btn-primary disabled" >Accepted from Finaleap</button>
														<?php
														}
														else{ ?>
...
$(document).ready(function () {
			...
			if(bank_id!="")
		{
			 ...
			 //  alert(bank_id);
				var url_corporate = window.location.origin+"/finaleap_finserv/dsa/dsa/index.php/Legal/get_coorporate_data";
				$.ajax({
					type:'POST',
					url:url_corporate,
				 ...
				 success:function(data){
									 ...
									 if(obj.FI_required=='yes')
									 {
										 //alert("hello");
										  $('#Legal').prop('disabled', false);
										 var url = window.location.origin+"/finaleap_finserv/dsa/dsa/index.php/Legal/get_FI_by_bank";
											$.ajax({
												 type:'POST',
												 url:url,
	...
	$( "#FI_name" ).change(function() 
	{
		 var value=$("#FI_name option:selected").text();
		 $('#FI_name_1').val(value);
	});
		
	
	$( "#revert_to_FI" ).click(function(e) {
  ...
  if(comment=='' || comment==' ')
  {
	...
	$("#FI_doc_upload").submit(function(e) {
									...
									var content='<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">'+
														'<div class="row " style="border: 1px solid #edeff1; margin-left:13px; margin-right:13px">'+
															'<div class="col-sm-10"><a href="'+value.path+'" target="_blank">  <i class="fa fa-file-text" style="color:green" aria-hidden="true"></i> &nbsp; <label for="email" style="font-size: 13px; color: black; font-weight: bold;">'+ value.Doc_name +'</label></a></div>'+
																...
																'<div class="col-sm-1">'+	
															       '<a style="margin-left: 8px; " href="'+window.location.origin+'/finaleap_finserv/dsa/dsa/index.php/customer/delete_vendor_doc/'+value.id+'"><i class="fa fa-trash text-right" style="color:red;"></i></a>'+
															'</div>'+			
														'</div></div></div>';
