			   ...
			   $Relationship=$this->Admin_model->bm_get_all_bureaupull($id);
				...
				$End_Date  = date('Y-m-t');
		         ...
		         $data['leadtoday']=$dashboard;*/
					...
					$dt1 = new \DateTime();
					$dt1->setTimestamp(1534694400);
					$dt=date('Y-m-01');
					
					$dt1= date('Y-m-t');
					
					$dts = new \DateTime();
					$dts->setTimestamp(1534694400);
					$dtd = new \DateTime();
					$dtd->setTimestamp(1534694400);
					$dts=date('Y-m-d');
					
					$dtd= date('Y-m-d');
					

				  $dashboard = $this->Admin_model->bm_current_month5($Relationship,$id,$dt,$dt1);
		         $data['bureau_pull']=$dashboard;
				 
				 
				 $dashboard = $this->Admin_model->bm_current_month5($Relationship,$id,$dts,$dtd);
		         $data['bureautoday']=$dashboard;
				 
				  $Start_Date = date('Y-m-01'); 
				$End_Date  = date('Y-m-t');
				  $sal1 = $this->Admin_model->bm_current_month($Relationship,$id,$Start_Date,$End_Date);
				// echo $sal1;
				 
				  $sal2 = $this->Admin_model->bm_current_month1( $Relationship,$id,$Start_Date,$End_Date);
				//  echo $sal2;
				 $dashboard=$sal1 +$sal2;
  ...
  public function BM_Lead_souring(){
	...
	// $Relationship=$this->Admin_model->bm_get_all_leadvisit($id);
	 if(isset($_GET['Start_Date']))
		...
		$Start_Date = date('Y-m-01'); 
				$End_Date  = date('Y-m-t');
				 $customers = $this->Admin_model->bm_current_month4($id,$Start_Date,$End_Date);
		         $arr['customers']=$customers;
...
public function BM_Lead_sourings()
 ...
 // exit;
  ...
  $searchQuery = " ";
  ## Total number of records without filtering
   $sel = $this->Admin_model->get_all_customers_Branch_lead($id);
/// print_r($sel);
//exit;
    // $sel=$this->Admin_model->Get_Total_No_Customer();
     $totalRecords =$sel;
     ## Total number of records with filtering
     $sel=$this->Admin_model->get_all_customers_Branch_filter_lead($id,$searchValue,$Start_Date,$End_Date);
	 //print_r( $sel);
	// exit;
     $totalRecordwithFilter =$sel;
     ## Fetch records
     $sel=$this->Admin_model->get_all_customers_Branch_filter_with_page_lead($id,$searchValue,$columnName,$columnSortOrder,$row,$rowperpage,$Start_Date,$End_Date);
     $empRecords = $sel;
     $data = array();
	...
	// $Relationship=$this->Admin_model->bm_get_all_leadvisit($id);
	 if(isset($_GET['Start_Date']))
        ...
        $data['row']=$this->Customercrud_model->getProfileData($id);
		 $Curr_Date = date('Y-m-d');
				 $Today_Date  = date('Y-m-d');
				 $today = $this->Admin_model->bm_current_month4($id,$Curr_Date,$Today_Date);
		         $arr['leadtoday']=$today;
...
//exit;
	...
	// exit;
     ...
     $empRecords = $sel;
     $data = array();
  $customer_created_by=$this->Admin_model->getCustomer_created_by();
        foreach($empRecords as $row)
        {
			foreach($customer_created_by as $row1)
			{ 	  
				  if($row->RO_ID!=NULL && $row->RO_ID!='0' )
				   {
          ...
          $data[] = array(
               "ID"=>$row->ID,
				...
				"Last_updated_on"=>$row->checked_dts,
               "FirstName"=>$row->FN,
			   "LastName"=>$row->LN,
