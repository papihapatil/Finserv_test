<?php
	...
	class Customer extends CI_Controller {
		...
		public function coapplicant_new_screen_two()
		{
			...
			$applicant_id = $coapp_data->CUST_ID;
			
			$data_applicant_income = $this->Customercrud_model->getCustomerIncomeInfo($applicant_id);
			
			//print_r($data_row_more);
			
			$data['coapp_data']=$coapp_data;
			
			$data['data_applicant_income']=$data_applicant_income;
				...
				else if($Documents->DOC_MASTER_TYPE=='INCOME PROOF')
				{
					if($inc_src=='self employeed')
						...
						else
						{
								...
								$mandatory_doc_count=$this->Customercrud_model->get_Mandatory_doc_count_for_employment($doc_type_user,$Documents->DOC_MASTER_TYPE,$inc_src);
								$savedDocType = $this->Customercrud_model->getSavedDocumentsCountWithMaster($id, $Documents->DOC_MASTER_TYPE);
								$savedoctype_details=$this->Customercrud_model->getSavedDocumentsCountWithMaster_details($id, $Documents->DOC_MASTER_TYPE);
								/*echo $mandatory_doc_count.'<br>';
								echo'<pre>';
								print_r($data_doctype);
								echo '<br>';
								echo'<pre>';
								print_r($savedoctype_details);
								echo '<br>';
								print_r($savedDocType);*/
								
								foreach($savedoctype_details as $details)
								{
                                     if($details->DOC_TYPE=='Salary Certificate')
									 {
                                           $mandatory_doc_count=1;
										   break;
									 }
									 else
									 {
		...
		public function lap_loan_doc()
		{
			...
			$savedDocType_details = $this->Customercrud_model->getSavedDocumentsCountWithMaster_details($id, "LAP");
			
			
			{
				foreach($savedDocType_details as $details)
				{
					if($details->DOC_TYPE=='8A'|| $details->DOC_TYPE=='Sale Deed')
					{
						
						$savedoc_count=$savedoc_count+1;
					}
				}
			}

			//$require_doc = $this->Customercrud_model->getDocumentscount_for_LAP($doc_type_user, "LAP");
			$require_doc = $this->Customercrud_model->get_Mandatory_doc_count_for_loan($doc_type_user, "LAP");
			
			
			
			if($savedoc_count >= $require_doc){
...
public function coapplicant_credit_score()
		...
		public function customer_apply_for_loan_with_score_credit()
		{
			...
			if($this->Customercrud_model->check_credit_score($id)){
				...
				$filterArr = [];
				$reditScoreSavedProfileData = $this->Customercrud_model->getCreditScoreSavedProfileData($id);
			  print_r($reditScoreSavedProfileData);
			  exit;
				if(!$reditScoreSavedProfileData || empty($reditScoreSavedProfileData)){
		...
		public function applied_loans_list()
		{
           ...
           //	echo $id;
			...
			$application_id ="FL".$loan_type.$result_id;
			$array_input_more = array(
												'CUST_ID' => $id,
...
$data['url'] = $url;
 ...
 /* code to upload file ends here */
		...
		public function customer_flow_update_s_one(){
			//$this->showErrorMessage("hi mahi");
			/*	echo $this->input->post('signature_name');
				echo $this->input->post('signature_name_want');
				echo $this->input->post('name_d_actual_name');
				echo $this->input->post('name_d_pan');
				echo $this->input->post('name_d_aadhar');
				echo $this->input->post('name_d_property');
				echo $this->input->post('name_d_befor_marriage');
				echo $this->input->post('birth_D_actual_dob');
				echo $this->input->post('birth_D_pan_dob');	
				echo $this->input->post('birth_D_aadhar_dob');
				echo $this->input->post('birth_D_inadvertently_dob');
				echo $this->input->post('language_D_applicant');
				echo $this->input->post('language_D_declarant');
				echo $this->input->post('language_D_employee_name');
				echo $this->input->post('language_D_employee_code');
				echo $this->input->post('language_D_declarant_recident_at');
				echo $this->input->post('language_D_Mr_Ms');		
				echo $this->input->post('language_D_in');
			

				*/	
					
				$signature_name_array= array(
												'signature_name'=>$this->input->post('signature_name'),
												'signature_name_want'=>$this->input->post('signature_name_want'),
											);
                $signature_name_array_JSON = json_encode($signature_name_array);	
					
				$name_d_array= array(
												'name_d_actual_name'=>$this->input->post('name_d_actual_name'),
												'name_d_pan'=>$this->input->post('name_d_pan'),
												'name_d_aadhar'=>$this->input->post('name_d_aadhar'),
												'name_d_property'=>$this->input->post('name_d_property'),
												'name_d_befor_marriage'=>$this->input->post('name_d_befor_marriage')
											);
                $name_d_array_JSON = json_encode($name_d_array);
			$id = $this->input->Post("ID");
		...
		public function customer_flow_update_s_two_salaried(){
			...
			if ($id == '') {
				...
				echo json_encode($response);
					...
					'clientlocation'=> $this->input->post('clientlocation'),
		...
		public function customer_flow_update_s_two_retired(){
			...
			if ($id == '') {
				...
				echo json_encode($response);
					...
					'retiredwith' => $this->input->post('retiredwith'),
					'retiredorhomemaker' => $this->input->post('retiredorhomemaker'),
				);	
...
//================================================================================================================================
		...
		public function coapplicant_new_screen_one_action(){
			...
			$u_unique_code = $this->input->post('COAPPID');
             ...
             if($profile_info->MOBILE != $array_input_more['MOBILE'])
			{
		  ...
		  if($profile_info->EMAIL != $array_input_more['EMAIL'])
			{
	   ...
	   public function customer_consent()
	{
	 ...
	 public function save_consent()
	 {
		   ...
		   // added by sonal -----------
			   ...
			   $consent=$this->input->post('is_agree');
			   $array_input['customer_consent'] = $consent;
			   $array_input['Sub_Stage'] ="Consent Recived";
			   $result=$this->Customercrud_model->update_profile($id, $array_input);
			   if($result>0)
			   {
					$this->session->set_flashdata('result', 'sucess');
...
public function coapplicant_new_screen_one_action_go_no_go()
{
	...
	$array_input_more = array(
		...
		'monthly_income'=>$this->input->post('monthly_income'),
		'current_emi'=>$this->input->post('current_emi'),
		'verify_aadhar_status'=>$this->input->post('aadhar_respone'),
	...
	public function coapplicant_documents()
		{
		   ...
		   // echo $inc_src;
			...
			{
				$Master_type_documen_customer=$this->Customercrud_model->getDocuments_Type_customer_Master_retired();
				//$Master_type_documen_customer=$this->Customercrud_model->getDocuments_Type_coapplicant_Master();
				//print_r($Master_type_documen_customer);
				//exit;
			...
			{
				//$getDocuments_Type_coapplicant_Master=$this->Customercrud_model->getDocuments_Type_customer_Master();//Distinct Master type from table Like Kyc ,residance,Employment	
				$Master_type_documen_customer=$this->Customercrud_model->getDocuments_Type_coapplicant_Master();
			}	
		...
		$bank_id=1;
		 ...
		 if(!empty($get_payment_deteils))
		 {
			 ...
			 if(count($get_payment_deteils)>0)
			 {
				...
				//redirect('customer/Declarations?UID='.$id);
			 }
			 else
			 {
				 ...
				 $array_input3 = array(
									'Stage'=>"Login Fee & Physical File Collection",
									'Sub_Stage'=>"Login Fee & Physical File Collection",
				                );
							    $updated_id = $this->Customercrud_model->update_profile($id, $array_input3);
				 //redirect('Admin/offline_payment_view?x='.base64_encode($id));
			 }
		 }
		 else
		 {
			 ...
			 if(!empty($get_payment_deteils_offline))
			 {
					...
					if($get_payment_deteils_offline->Payment_Recived_date=='')
					{
					   $data_row = $this->Customercrud_model->getAppliedLoans($id);
					   $data['showNav'] = 1;
					   $data['age'] = $age;
...
public function delete_sanction_doc(){
			
			$id = $this->input->get("UID");
						
			$array_input = array(
				'ID' => $this->input->get("DOC")	
			);		
...
public function view_vendors_cloud_file($fileid)
		{

						$user = $this->Customercrud_model->get_vendors_doc($fileid);
...
public function delete_vendor_doc($docid){
...
public function delete_vendor_doc_RCU($docid){
...
public function delete_vendor_doc_Legal($docid){
...
public function delete_vendor_doc_Technical($docid){
   ...
   public function view_Disbursement_file($fileid)
		{

						$user = $this->Customercrud_model->getdisbursementDoc($fileid);
		...
		public function view_waiver_file($fileid)
		{

				$user = $this->Customercrud_model->getSingleDoc($fileid);

				$documentname = $user->Waiver_doc_cloud_dir.$user->Waiver_doc_cloud_name;

				$pathname = "cloudfile/".$user->Waiver_doc_cloud_name;
		...
		public function update_aadhar_response()
		{
			$ID=$this->input->post('ID');
			$verify_aadhar_status=$this->input->post('verify_aadhar_status');
			$array_input = array(
			
				'verify_aadhar_status'=>$verify_aadhar_status
				);
			$this->Customercrud_model->update_profile($ID, $array_input);
		}
		//===================== added by priya ==================================================16-11-2022==========================
		public function view_sanction_document($fileid)
		{
			$user = $this->Customercrud_model->ShowSanctionCondition($fileid);
					
						$documentname = $user->Document_path.$user->Documnt_name;

						$pathname = "cloudfile/".$user->Documnt_name;
		...
		public function view_sanction_document_approval_email($fileid)
		{
			$user = $this->Customercrud_model->ShowSanctionCondition($fileid);
					
						$documentname = $user->OTC_PDD_document_path.$user->OTC_PDD_document;

						$pathname = "cloudfile/".$user->OTC_PDD_document;
		...
		//========================== added by priya 04-01-2022
		public function view_legal_document($fileid)
		{
			
			$user = $this->credit_manager_user_model->ShowLegalCondition($fileid);
		...
		public function view_legal_document_approval_email($fileid)
		{
			$user = $this->credit_manager_user_model->ShowLegalCondition($fileid);
		...
		public function Declarations()
		{
			...
			$age = $this->session->userdata("AGE");
			$id = $this->input->get("UID");
			$loans = $this->Customercrud_model->getAppliedLoans($id);
		    $data_row = $this->Customercrud_model->getProfileData($id);
		    $co_app = $this->Customercrud_model->getMyCoapplicants($id);
		    $result_id_row = $this->Customercrud_model->find_customer_entry_id($id);
			$var=$result_id_row->ID;
			...
			$this->load->view('header', $data);
			$this->load->view('customer/Declarations1',$data);
			//$this->load->view('customer/applied_loans');
		}
