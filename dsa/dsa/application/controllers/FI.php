...
class FI extends CI_Controller 
    {
		...
		public function index(){
			...
			if($this->session->userdata("USER_TYPE") == ''){
                ...
                // ==========================================================
                $dashboard_data = $this->Legal_model->getDashboardData_FI($id);    
                
            
                $data['dashboard_data'] = $dashboard_data;
                $this->load->view('dashboard_header', $data);
                $this->load->view('FI/dashboard', $data); 
	...
	public function customers()
	{
			...
			if($this->session->userdata('USER_TYPE') == '' || $this->session->userdata('USER_TYPE') == 'CUSTOMER'){
				...
				$arr['s'] = $filter;
				$this->load->view('dashboard_header', $data);
				$this->load->view('FI/customers', $arr);
			}
		}
		...
		public function View_Details_Customer()
		{
			 ...
			 $this->load->helper('url');
			 $data['userType'] = $this->session->userdata("USER_TYPE");
			   $data_row = $this->credit_manager_user_model->getDocuments_send_to_legal_by_CPA($id,'FI');
                $age = $this->session->userdata('AGE');
                $data['showNav'] = 1;
                $data['age'] = $age;
                $data['userType'] = $this->session->userdata("USER_TYPE");
               ...
               $USER_TYPE = $this->session->userdata("USER_TYPE");
				...
				$data['Legal_docs_info']=$this->Legal_model->get_fi_docs($data['cust_info']->Application_id);
				$data['documents']=$data_row;
                $data ['Legals']=$this->Legal_model->getLegals();
                $this->load->view('dashboard_header', $data);
                $this->load->view('FI/view_details_customer', $data);
		}
		...
		public function Send_Documents()
		{
		   ...
		   $data['cust_info']=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_id);
			...
			$this->load->helper('url');
			   ...
			   $data['Applicant_data']=$this->Legal_model->get_fi_docs($Application_id);
			   
			   $this->load->view('dashboard_header', $data);
			   $this->load->view('FI/send_documents', $data);
		}
  ...
  font-family: Arial, sans-serif;
	...
	public function FI_remark()
		{
			...
			if(!empty(  $additional_emi_comments3))
			{
			 ...
			 $data_doc['additional_comments']=$additional_comments;
						...
						$data_doc['fi_remark']=$this->input->post("remark");
						$data_doc['fi_status']='Document Received From FI';
						$data_doc['last_updated_by_fi']=date("d-m-Y h:i:sa");
						
						$notification_data=['user_id'=>$Credit_id,'notification'=>'Document Received From FI :'.$row->FN.' '.$row->LN,'status'=>'unseen'];
			                  $notification=$this->Admin_model->insert_notification($notification_data);
					
			
			//$result_doc1=$this->credit_manager_user_model->Update_Legal_status($UNIQUE_CODE,$data_status);
			$result_docs = $this->credit_manager_user_model->Update_Fi_details($id,$data_doc);
			if($result_docs>0)
			{
			...
			$this->session->set_userdata("result1", 1);
					...
					$this->sendEmail_all_Document_Recived_FI($user_info,$cust_info,$id);
			
			/*---------------------------------------------------------------------------------------------*/
			redirect('FI/Send_Documents?x='.base64_encode($id).'&y='.base64_encode($UNIQUE_CODE));
    ...
    public function customers_PG()
    {
        ...
        $sel = $this->Dsacustomers_model->getALLCustomers_FI($id,$userType);
       // $sel=$this->Admin_model->Get_Total_No_Customer();
        ...
        ## Fetch records
        $sel=$this->Dsacustomers_model->Get_ALLCustomer_With_Page_FI($id,$searchValue,$columnName,$columnSortOrder,$row,$rowperpage);
        $empRecords = $sel;
        $data = array();

        foreach($empRecords as $row){
			if ($row->fi_status=='Accepted from Finaleap') {
			...
			else{ $view='<a href="'. base_url().'index.php/FI/View_Details_Customer?x='.base64_encode($row->Application_id).'"  class="btn modal_test"><i class="fa fa-paper-plane text-center" style="color:blue;"></i></a>';
				$document='<a href="'.base_url().'index.php/FI/Send_Documents?x='.base64_encode($row->Application_id).' &y='. base64_encode($row->UNIQUE_CODE).'"  class="btn modal_test"><i class="fa fa-envelope text-center" style="color:blue;"></i></a>';
			}
            ...
            $data[] = array(
                    ...
                    "Created_on"=>$row->fi_status,
                    "Last_updated_on"=>$row->fi_remark,
                    "UNIQUE_CODE"=>$view,
	...
	{   
		...
		$cust_info=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_ID);
		    ...
		    $notification_data=['user_id'=>$row->UNIQUE_CODE,'notification'=>'Customer Revert From FI:'.$cust_name,'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data);	
			
		  $result_docs = $this->credit_manager_user_model->Update_Fi_details($Application_ID,$data_doc);
	
			
			$this->SendEmail_to_team_revert_FI($Application_ID,$cust_info);
	...
	public Function SendEmail_to_team_revert_FI($Application_id,$cust_info)
	  {
	...
	$send_to_emails=$send_to_emails.',sachin.kardile@finaleap.com,credit@fianleap.com,operations@finaleap.com';
			...
			$this->email->subject('Alert : Query Raised ---- FI Report  ----'.$Application_id.' ---- '.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN));
			$data['cust_info']=$cust_info;
			$mailContent = $this->load->view('templates/FI_revert_to_Team',$data,true);
				...
				//echo "<br>";
				 ...
				 /* code to export files to Nextcloud starts here */
				   ...
				   (
					...
					$data_row_FI = $this->credit_manager_user_model->getDocuments_send_to_legal($Application_id,'FI','CPA');	
	...
	public function sendEmail_all_Document_Recived_FI($Legal_info,$cust_info,$Application_ID)
	  {
		$Eamils=array();
		$Appication_info=$this->Legal_model->get_fi_docs($Application_ID);
		$Operation_details=$this->Customercrud_model->getProfileData($Appication_info->credit_manager_id);
	...
	//$send_to_emails='papiha.patil@finaleap.com';
			...
			$this->email->subject($Application_ID.'----'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'---- FI REPORT'); 
		    $data['Legal_info']=$Legal_info;
			
			$data['cust_info']=$cust_info;
			$mailContent = $this->load->view('templates/FI_document_recived_team',$data,true);
			$this->email->message($mailContent); 
			if($this->email->Send())
			{
