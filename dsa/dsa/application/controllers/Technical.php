...
class Technical extends CI_Controller 
    {
		...
		public function index(){
			if($this->session->userdata("USER_TYPE") == ''){
                ...
                // ==========================================================
                $dashboard_data = $this->Legal_model->getDashboardData_Technical($id);    
                
            
                $data['dashboard_data'] = $dashboard_data;
                $this->load->view('dashboard_header', $data);
                $this->load->view('Technical/dashboard', $data); 
	...
	public function customers()
	{
			...
			if($this->session->userdata('USER_TYPE') == '' || $this->session->userdata('USER_TYPE') == 'CUSTOMER'){
				...
				$this->load->view('Technical/customers', $arr);
			}
		}
		...
		public function View_Details_Customer()
		{
			 ...
			 $this->load->helper('url');
			 $data['userType'] = $this->session->userdata("USER_TYPE");
			   $data_row = $this->credit_manager_user_model->getDocuments_send_to_legal_by_CPA($id,'Technical');
                $age = $this->session->userdata('AGE');
                $data['showNav'] = 1;
                $data['age'] = $age;
                $data['userType'] = $this->session->userdata("USER_TYPE");
               ...
               $USER_TYPE = $this->session->userdata("USER_TYPE");
				...
				$data['Legal_docs_info']=$this->Legal_model->get_Technical_docs($data['cust_info']->Application_id);
				$data['documents']=$data_row;
                $data ['Legals']=$this->Legal_model->getLegals();
                $this->load->view('dashboard_header', $data);
                $this->load->view('Technical/view_details_customer', $data);
		}
			...
			$Results=array();
             $GLOBALS['msg'] = ""; 
                 for($i=0;$i<$count;$i++)
				 {
					...
					/*if (!file_exists('./images/Legal_Documents/'.$id.".".pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION)) ){   
	...
	/*------------------------------------------ Added by papiha on 23_03_2022-----------------------------------------------------------*/
		...
		public function get_Technical_by_bank()
		{
			$bank_id=$this->input->post('bank_id');
			$Legal_names=$this->Legal_model->get_Technical_by_bank($bank_id);
		...
		public function Send_Documents()
		{
		   ...
		   $data['cust_info']=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_id);
			...
			$this->load->helper('url');
			   ...
			   $data['Applicant_data']=$this->Legal_model->get_Technical_docs($Application_id);
			   
			   $this->load->view('dashboard_header', $data);
			   $this->load->view('Technical/send_documents', $data);
		}
  ...
  font-family: Arial, sans-serif;
	...
	public function Technical_remark()
		{
			...
			$additional_emi_comments3=$this->input->post('additional_emi_comments_3');
			   if(!empty(  $additional_emi_comments3))
			   {
				...
				else
				{
						...
						$data_doc['Technical_status']='Document Received From Technical';
						$data_doc['last_updated_by_Technical']=date("d-m-Y h:i:sa");
						$data_doc['additional_comments']=$additional_comments;
						
						$notification_data=['user_id'=>$Credit_id,'notification'=>'Document Received From Technical :'.$row->FN.' '.$row->LN,'status'=>'unseen'];
			                  $notification=$this->Admin_model->insert_notification($notification_data);
					
			
			//$result_doc1=$this->credit_manager_user_model->Update_Legal_status($UNIQUE_CODE,$data_status);
			$result_docs = $this->credit_manager_user_model->Update_Technical_details($id,$data_doc);
			if($result_docs>0)
			{
			...
			$this->session->set_userdata("result1", 1);
			       ...
			       $user_info=$this->Customercrud_model->getProfileData($Legal_id);
					...
					$this->sendEmail_all_Document_Recived_Technical($user_info,$cust_info,$id);
			
			/*---------------------------------------------------------------------------------------------*/
			redirect('Technical/Send_Documents?x='.base64_encode($id).'&y='.base64_encode($UNIQUE_CODE));
    ...
    public function customers_PG()
    {
        ...
        $sel = $this->Dsacustomers_model->getALLCustomers_Technical($id,$userType);
       // $sel=$this->Admin_model->Get_Total_No_Customer();
        $totalRecords =$sel;
        ## Total number of records with filtering
        $sel=$this->Dsacustomers_model->Get_ALLCustomer_Filter_Technical($id,$searchValue);
        $totalRecordwithFilter =$sel;
        ## Fetch records
        $sel=$this->Dsacustomers_model->Get_ALLCustomer_With_Page_Technical($id,$searchValue,$columnName,$columnSortOrder,$row,$rowperpage);
        $empRecords = $sel;
        $data = array();

        foreach($empRecords as $row){
			if ($row->Technical_status=='Accepted from Finaleap') {
			...
			else{ $view='<a href="'. base_url().'index.php/Technical/View_Details_Customer?x='.base64_encode($row->Application_id).'"  class="btn modal_test"><i class="fa fa-paper-plane text-center" style="color:blue;"></i></a>';
				$document='<a href="'.base_url().'index.php/Technical/Send_Documents?x='.base64_encode($row->Application_id).' &y='. base64_encode($row->UNIQUE_CODE).'"  class="btn modal_test"><i class="fa fa-envelope text-center" style="color:blue;"></i></a>';
			}
            ...
            $data[] = array(
                    ...
                    "Created_on"=>$row->Technical_status,
                    "Last_updated_on"=>$row->Technical_remark,
                    "UNIQUE_CODE"=>$view,
	...
	/*------------------------- Addded by papiha on 02_08_2022-------------------------------*/
	...
	{   
		...
		$cust_info=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_ID);
		  ...
		  $result_docs = $this->credit_manager_user_model->Update_Technical_details($Application_ID,$data_doc);
			...
			$this->SendEmail_to_team_revert_Technical($Application_ID,$cust_info);
	...
	public Function SendEmail_to_team_revert_Technical($Application_id,$cust_info)
	  {
	...
	//$send_to_emails=$send_to_emails.',sachin.kardile@finaleap.com,sunil.kalan@finalep.com,rupali.more@finaleap.com';
	...
	$send_to_emails=$send_to_emails.',sachin.kardile@finaleap.com,credit@fianleap.com,operations@finaleap.com';
			...
			$mailContent = $this->load->view('templates/Technical_revert_to_Team',$data,true);
			...
			if(isset($_POST['upload']))
			{
				...
				//echo "<br>";
				 ...
				 /* code to export files to Nextcloud starts here */
				   ...
				   $correcturl = "curl -X PUT -u jaynimbalkar:Jay@1985 https://files.finaleap.com/remote.php/dav/files/jaynimbalkar/".$filenamewithdir." -T ".$_FILES["userfile"]["tmp_name"]."";
				   exec($correcturl);
				   $file_input_arr = array
				   (
					'Application_id' => $Application_id,
					'Doc_name' => 'FI document',	
					'path' =>$dirname,
					'User'=>$u_type,
					...
					redirect('Technical/Send_Documents?x='.base64_encode($Application_id).'&y='.$id);
				}
			
		}
		public function delete_FI_doc($docid){
			...
			redirect('FI/Send_Documents?x='.base64_encode($Application_id).'&y='.$id);
		
		}
	public function sendEmail_all_Document_Recived_Technical($Legal_info,$cust_info,$Application_ID)
	  {
		$Eamils=array();
	...
	//$send_to_emails='papiha.patil@finaleap.com';
			...
			$mailContent = $this->load->view('templates/Technical_document_recived_team',$data,true);
			$this->email->message($mailContent); 
			if($this->email->Send())
			{
	...
	/*---------------------------------- addded by papiha on 11_08_2022 for <technical--------------------------------------></technical--------------------------------------*/
	function do_upload_Technical_doc()
		{
				...
				$dirname = "Finaleap/Technical_documents/".$id;
				...
				//echo "<br>";
				 ...
				 /* code to export files to Nextcloud starts here */
				   ...
				   (
					...
					$this->credit_manager_user_model->saveDocuments($file_input_arr);
					//redirect('Technical/Send_Documents?x='.base64_encode($Application_id).'&y='.base64_encode($id));
					$data_row_FI = $this->credit_manager_user_model->getDocuments_send_to_legal($Application_id,'Technical','CPA');	
					$response = array('result' => 3,'docs'=>$data_row_FI);
					echo json_encode($response);
				
			
		}
		public function delete_Technical_doc($docid,$UNIQUE_CODE)
		{
				...
				$u_type = $this->session->userdata("USER_TYPE");
				$result = $this->Customercrud_model->delete_vendor_doc($array_input);
				redirect('Technical/Send_Documents?x='.base64_encode($Application_id).'&y='.base64_encode($UNIQUE_CODE));
