...
class RCU extends CI_Controller 
    {
		...
		public function index(){
			if($this->session->userdata("USER_TYPE") == ''){
                ...
                // ==========================================================
                $dashboard_data = $this->Legal_model->getDashboardData_RCU($id);    
                
            
                $data['dashboard_data'] = $dashboard_data;
                $this->load->view('dashboard_header', $data);
                $this->load->view('RCU/dashboard', $data); 
	...
	public function customers()
	{
		...
		public function View_Details_Customer()
		{
			 ...
			 $this->load->helper('url');
			 $data['userType'] = $this->session->userdata("USER_TYPE");
			   $data_row = $this->credit_manager_user_model->getDocuments_send_to_legal_by_CPA($id,'RCU');
                $age = $this->session->userdata('AGE');
                $data['showNav'] = 1;
                $data['age'] = $age;
                $data['userType'] = $this->session->userdata("USER_TYPE");
               ...
               $USER_TYPE = $this->session->userdata("USER_TYPE");
				...
				$data['Legal_docs_info']=$this->Legal_model->get_RCU_docs($data['cust_info']->Application_id);
				$data['documents']=$data_row;
                $data ['Legals']=$this->Legal_model->getLegals();
                $this->load->view('dashboard_header', $data);
                $this->load->view('RCU/view_details_customer', $data);
		}
	...
	/*------------------------------------------ Added by papiha on 23_03_2022-----------------------------------------------------------*/
		...
		public function Send_Documents()
		{
		   ...
		   $data['cust_info']=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_id);
			...
			$USER_TYPE = $this->session->userdata("USER_TYPE");
			$ID=$this->session->userdata('ID');
			$data_row = $this->credit_manager_user_model->getDocuments_send_to_legal($Application_id,'RCU','CPA');
			$this->load->helper('url');
			   $age = $this->session->userdata('AGE');
			   $data['showNav'] = 1;
			   $data['age'] = $age;
			   ...
			   $data['Applicant_data']=$this->Legal_model->get_RCU_docs($Application_id);
			   
			   $this->load->view('dashboard_header', $data);
			   $this->load->view('RCU/send_documents', $data);
		}
  ...
  font-family: Arial, sans-serif;
	...
	public function RCU_remark()
		{
			...
			if(!empty(  $additional_emi_comments3))
			{
			 ...
			 $data_doc['additional_comments']=$additional_comments;
						...
						$data_doc['RCU_remark']=$this->input->post("remark");
						$data_doc['RCU_status']='Document Received From RCU';
						$data_doc['last_updated_by_RCU']=date("d-m-Y h:i:sa");
						
						$notification_data=['user_id'=>$Credit_id,'notification'=>'Document Received From RCU :'.$row->FN.' '.$row->LN,'status'=>'unseen'];
			                  $notification=$this->Admin_model->insert_notification($notification_data);
					
			
			//$result_doc1=$this->credit_manager_user_model->Update_Legal_status($UNIQUE_CODE,$data_status);
			$result_docs = $this->credit_manager_user_model->Update_RCU_details($id,$data_doc);
			if($result_docs>0)
			{
			...
			$this->session->set_userdata("result1", 1);
			       ...
			       $user_info=$this->Customercrud_model->getProfileData($Legal_id);
					...
					$this->sendEmail_all_Document_Recived_RCU($user_info,$cust_info,$id);
			
			/*---------------------------------------------------------------------------------------------*/
			redirect('RCU/Send_Documents?x='.base64_encode($id).'&y='.base64_encode($UNIQUE_CODE));
    ...
    public function customers_PG()
    {
        ...
        $sel = $this->Dsacustomers_model->getALLCustomers_RCU($id,$userType);
       // $sel=$this->Admin_model->Get_Total_No_Customer();
        $totalRecords =$sel;
        ## Total number of records with filtering
        $sel=$this->Dsacustomers_model->Get_ALLCustomer_Filter_RCU($id,$searchValue);
        $totalRecordwithFilter =$sel;
        ## Fetch records
        $sel=$this->Dsacustomers_model->Get_ALLCustomer_With_Page_RCU($id,$searchValue,$columnName,$columnSortOrder,$row,$rowperpage);
        $empRecords = $sel;
        $data = array();

        foreach($empRecords as $row){
           
			if ($row->RCU_status=='Accepted from Finaleap') {
			...
			else{ $view='<a href="'. base_url().'index.php/RCU/View_Details_Customer?x='.base64_encode($row->Application_id).'"  class="btn modal_test"><i class="fa fa-paper-plane text-center" style="color:blue;"></i></a>';
				$document='<a href="'.base_url().'index.php/RCU/Send_Documents?x='.base64_encode($row->Application_id).' &y='. base64_encode($row->UNIQUE_CODE).'"  class="btn modal_test"><i class="fa fa-envelope text-center" style="color:blue;"></i></a>';
			}
            $data[] = array(
                    "ID"=>$row->ID,
                    ...
                    "Created_on"=>$row->RCU_status,
                    "Last_updated_on"=>$row->RCU_remark,
                    "UNIQUE_CODE"=>$view,
	...
	{   
		...
		$cust_info=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_ID);
		    ...
		    $notification_data=['user_id'=>$row->UNIQUE_CODE,'notification'=>'Customer Revert From RCU:'.$cust_name,'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data);	
			
		  $result_docs = $this->credit_manager_user_model->Update_RCU_details($Application_ID,$data_doc);
	
			
			$this->SendEmail_to_team_revert_RCU($Application_ID,$cust_info);
	...
	public Function SendEmail_to_team_revert_RCU($Application_id,$cust_info)
	  {
	...
	$send_to_emails=$send_to_emails.',sachin.kardile@finaleap.com,credit@fianleap.com,operations@finaleap.com';
			...
			$this->email->subject('Alert : Query Raised ---- RCU Report  ----'.$Application_id.' ---- '.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN));
			$data['cust_info']=$cust_info;
			$mailContent = $this->load->view('templates/RCU_revert_to_Team',$data,true);
		...
		function do_upload_RCU_doc_RCU()
		{
				...
				//echo "<br>";
				 ...
				 /* code to export files to Nextcloud starts here */
				   ...
				   (
					...
					//redirect('RCU/Send_Documents?x='.base64_encode($Application_id).'&y='.$id);
					$data_row_FI = $this->credit_manager_user_model->getDocuments_send_to_legal($Application_id,'RCU','CPA');	
					$response = array('result' => 3,'docs'=>$data_row_FI);
					echo json_encode($response);
				
			
		}
		public function delete_RCU_doc($docid,$unique_code){
			...
			redirect('RCU/Send_Documents?x='.base64_encode($Application_id).'&y='.base64_encode($unique_code));
		
		}
	public function sendEmail_all_Document_Recived_RCU($Legal_info,$cust_info,$Application_ID)
	  {
		$Eamils=array();
	...
	//$send_to_emails='papiha.patil@finaleap.com';
			...
			$this->email->subject($Application_ID.'----'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'---- RCU REPORT'); 
		 
			$data['cust_info']=$cust_info;
			$data['Legal_info']=$Legal_info;
			$mailContent = $this->load->view('templates/RCU_document_recived_team',$data,true);
			$this->email->message($mailContent); 
			if($this->email->Send())
			{
