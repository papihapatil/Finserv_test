...
class Credit_manager_user extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->session->userdata("USER_TYPE") == ''){
			...
			$dashboard_data_credit_ops = $this->Operations_user_model->getDashboardData();
			$data['dashboard_data_credit_ops'] = $dashboard_data_credit_ops;
			$data['row']=$this->Customercrud_model->getProfileData($id);
			$data['dashboard_data'] = $dashboard_data;
			$filter_by_credit_ops ='all';
			$dashboard_data_2 = $this->Operations_user_model->getDashboardData_2($filter_by_credit_ops);   
		  ...
		  if($this->session->userdata("USER_TYPE") == ''){
			...
			else
			{
				...
				$data_coapplicant=$this->Operations_user_model->getCoapplicantById($id);
					...
					if(isset($coapp_data_credit_analysis_array))
					{
				$data['coapp_data_credit_analysis_array']=$coapp_data_credit_analysis_array;
					}
					else
					{
						$data['coapp_data_credit_analysis_array']=array();
		  ...
		  $arrContextOptions=array(
           "ssl"=>array(
               ...
               "verify_peer_name"=>false,
		   	  	  ...
		   	  	  $mpdf->SetWatermarkText('Finaleap Finserv');
...
/*----------------------------- Techical Flow adde by papiha------------------------------------------------*/
   ...
   // $sel=$this->Admin_model->Get_Total_No_Customer();
	...
	foreach($empRecords as $row){
		     ...
		     $find_my_legal_documents=$this->credit_manager_user_model->find_my_legal_documents($row->UNIQUE_CODE);
			 $find_my_technical_documents=$this->credit_manager_user_model->find_my_technical_documents($row->UNIQUE_CODE);
			 $find_my_FI_documents=$this->credit_manager_user_model->find_my_FI_documents($row->UNIQUE_CODE);
			 $find_my_RCU_documents=$this->credit_manager_user_model->find_my_RCU_documents($row->UNIQUE_CODE);
			 $find_my_Court_case_documents=$this->credit_manager_user_model->find_my_Court_case_documents($row->UNIQUE_CODE);
			 $find_my_Legal_Document_Search_documents=$this->credit_manager_user_model->find_my_Legal_Document_Search_documents($row->UNIQUE_CODE);
			 if(count($find_my_legal_documents)>0 && count($find_my_technical_documents)>0 &&  count($find_my_FI_documents)>0 && count($find_my_RCU_documents)>0 && count($find_my_Court_case_documents)>0 && count($find_my_Legal_Document_Search_documents)>0)
			 {
                  $reports='<a href="'.base_url().'index.php/credit_manager_user/Vendors_Roports?x='.base64_encode($row->Application_id).'" class="btn  btn-success modal_test">Reports</a>';
			 }
			 else
			 {
				$reports='<a href="'.base_url().'index.php/credit_manager_user/Vendors_Roports?x='.base64_encode($row->Application_id).'" class="btn modal_test">Reports</a>';
			 }
		...
		$data[] = array(
				...
				<br><a href="'.base_url().'index.php/credit_manager_user/Vendors_Roports?x='.base64_encode($row->Application_id).'" class="btn modal_test">Reports</a>',	*/
				"SendToVendors"=>$reports,
				"PD"=>$PD,
...
foreach($empRecords as $row){
	...
	$find_my_RCU_documents=$this->credit_manager_user_model->find_my_RCU_documents($row->UNIQUE_CODE);
	$find_my_Court_case_documents=$this->credit_manager_user_model->find_my_Court_case_documents($row->UNIQUE_CODE);
	$find_my_legal_vetting_documents=$this->credit_manager_user_model->find_my_legal_vetting_documents($row->UNIQUE_CODE);
	if(count($find_my_legal_documents)>0 && count($find_my_technical_documents)>0 &&  count($find_my_FI_documents)>0 && count($find_my_RCU_documents)>0 && count($find_my_Court_case_documents)>0 && count($find_my_legal_vetting_documents)>0)
	{
		 $reports='<a href="'.base_url().'index.php/credit_manager_user/Vendors_Roports?x='.base64_encode($row->Application_id).'" class="btn  btn-success modal_test">Reports</a>';
	}
	else
...
';
	...
	$data[] = array(
			...
			"EMAIL"=>'<a href="'.base_url().'index.php/Admin/Customer_Summary?ID='.$row->UNIQUE_CODE.'" target="_blank">'.$row->EMAIL.'</a>',
			"MOBILE"=>'<a href="'.base_url().'index.php/Admin/Customer_Summary?ID='.$row->UNIQUE_CODE.'" target="_blank">'.$row->MOBILE.'</a>',
			...
			"journy_status"=>'<a href="'.base_url().'index.php/Admin/Customer_Summary?ID='.$row->UNIQUE_CODE.'" target="_blank">'.$journy.'</a>',
			"Date"=>'<a href="'.base_url().'index.php/Admin/Customer_Summary?ID='.$row->UNIQUE_CODE.'" target="_blank">'.$row->CREATED_AT.'</a>',
			"Bureau"=>$BUREAU,
			"pre_cam"=>$preCam,
			"CAM"=>$Cam_pdf,
			"pd"=>$pd,
 ...
 public function Sanction_Letter()
     ...
     public function submit_sanction_letter_form_two()
	{
        ...
        $this->email->set_newline("\r\n");
			...
			$send_email_to_3_HR='shiv@finaleap.com,amit@finaleap.com,vipul@finaleap.com,sachin.kardile@finaleap.com,sunil.kalan@finaleap.com';
	   ...
	   public function Sanction_Documents()
	  {
		  ...
		  if( $id1=='') 
		  ...
		  else
			{
				...
				$data['find_my_uploded_legal_conditions']=$this->credit_manager_user_model->find_my_uploded_legal_conditions($id);
				...
				$data['post_sanction_document_list']=$this->credit_manager_user_model->post_sanction_document_list($id); 
				
				//======================= added by priya 06-01-2023================================================================
				...
				$data['find_my_Pre_CERSAI_letter']=$this->credit_manager_user_model->find_my_Pre_CERSAI_letter($id);
				//echo "<pre>";
				//print_r($data['find_my_uploded_sanction_conditions']);
				//echo "</pre>";
				//exit();
				if(!empty($data_row_pd_table))
				{
...
//$send_to_emails='papiha.patil@finaleap.com';
          $send_to_cc=$send_to_cc.',sachin.kardile@finaleap.com,sunil.kalan@finaleap.com,misuser@finaleap.com';
	    $data_row_FI = $this->credit_manager_user_model->getDocuments_send_to_legal($Application_id,'','FI');
		...
		$this->email->subject('FI REPORT INITIATE'.'----'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'----'.$Application_id); 
...
//$send_to_emails='papiha.patil@finaleap.com';
		...
		$this->email->to($send_email_to_support);
		$this->email->cc($send_to_cc);
		$this->email->subject('RCU REPORT INITIATE'.'----'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'----'.$Application_id); 
		$data['cust_info']=$cust_info;
...
//$send_to_emails='papiha.patil@finaleap.com';
		...
		$this->email->subject('Legal REPORT INITIATE'.'----'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'----'.$Application_id); 
		$data['cust_info']=$cust_info;
		$data['documents']=$documents;
...
function do_upload_Technical_doc()
  ...
  public Function SendEmail_To_Technical($FI_EMAIL,$Application_id,$cust_info,$documents)
  {
	...
	if($cust_info->Location=='Satara'||$cust_info->Location=='Sangli')
	{
		$Location="Pune";
		$get_credit=$this->Operations_user_model->getcredit_by_location($Location);
	}
	else{
    $get_credit=$this->Operations_user_model->getcredit_by_location($cust_info->Location);
	}
...
//$send_to_emails='papiha.patil@finaleap.com';
		...
		$this->email->to($send_email_to_support);
		$this->email->cc($send_to_cc);
		$this->email->subject('TECHNICAL REPORT INITIATE'.'----'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'----'.$Application_id); 
		$data['cust_info']=$cust_info;
		$data['documents']=$documents;
	...
	public function Disbursement_pending_documents_PG()
{
	...
	$sel = $this->credit_manager_user_model->getPending_doc_count();
	$totalRecords =$sel;
	$sel=$this->credit_manager_user_model->searchPending_document($searchValue);
	$totalRecordwithFilter =$sel;
	$sel=$this->credit_manager_user_model->getPending_document_With_Page($searchValue,$columnName,$columnSortOrder,$row,$rowperpage);
	$empRecords = $sel;
	...
	{	
	   // $edit ='<a href="'. base_url().'index.php/Credit_manager_user/Edit_pending_documents?I='. base64_encode($row->UNIQUE_CODE).'"Class="btn modal_test"><i class="fa fa-edit text-right" style="color:blue;"></i></a>';
         $edit= '<a class="dropdown-item" href="'. base_url().'index.php/credit_manager_user/Sanction_Documents?ID='.($row->UNIQUE_CODE).'&CM=" target="_blank">
           			<button type="button" class="btn btn-success">View Documents </button> 
           		</a>';
         if($row->Credit_proceed_status == "Confirmed")		
		 {
			 $status="Sent for Disbursement";
		 }
		  else if($row->Credit_proceed_status == "Reverted")		
		 {
			 $status="Reverted";
		 }
		 else
	...
	public function save_FI_document()
			...
			public function save_Report_document()
			{
				...
				if(!empty($_FILES['file']['name']))
				{
						...
						else if($data['select_repot_name']== "Legal Vetting Report")
						{
							$Doc_type = "Legal Vetting Report";
							$dirname = "Finserv/Legal_Vetting_Report/".$id;
						}
						...
						else if($data['select_repot_name'] == "Loan Application Form")
						{
							$Doc_type = "Loan Application Form";
							$dirname = "Finserv/LoanApplicationForm/".$id;
						}
						else if($data['select_repot_name'] == "Signed Loan Agreement" )
						{
							
							$Doc_type = "Signed Loan Agreement";
							$dirname = "Finserv/Loan_agreement/".$id;
						}
						else if($data['select_repot_name'] == "Signed Sanction Letter")
						{
												
							$Doc_type = "Signed Sanction Letter";
							$dirname = "Finserv/Loan_agreement/".$id;
						}	
						else if($data['select_repot_name'] == "Signed MITC Letter")
						{
												
							$Doc_type = "Signed MITC Letter";
							$dirname = "Finserv/Loan_agreement/".$id;
						}
						else if($data['select_repot_name'] == "Bank Signature Verification")
						{
												
							$Doc_type = "Bank signature verification";
							$dirname = "Finserv/Loan_agreement/".$id;
						}	
						else if($data['select_repot_name'] == "Vernacular Declaration")
						{
												
							$Doc_type = "Vernacular Declaration";
							$dirname = "Finserv/Loan_agreement/".$id;
						}	
						else if($data['select_repot_name'] == "Dual Name Declatarion")
						{
												
							$Doc_type = "Dual name declatarion";
							$dirname = "Finserv/Loan_agreement/".$id;
						}	
						else if($data['select_repot_name'] == "End Use Letter")
						{
												
							$Doc_type = "End use letter";
							$dirname = "Finserv/Loan_agreement/".$id;
						}	
						else if($data['select_repot_name'] == "Pre EMI Letter")
						{
												
							$Doc_type = "Pre EMI letter";
							$dirname = "Finserv/Loan_agreement/".$id;
						}	
						else if($data['select_repot_name'] == "Pre CERSAI")
						{
						 ...
						 if($data['select_repot_name']== "CAM Excel")
						{
							...
							$find_CAM_excel = $this->Customercrud_model->find_CAM_excel($id);
							if(!empty($find_CAM_excel ))
							{
							
								$docid = $this->Customercrud_model->update_CAM_excel($id,$file_input_arr);
							}
							else
							{
						...
						else
						{
								...
								'doc_cloud_name' => $filename,
								'Cust_Id'=>$id,
								'uploded_doc'=>date("d-m-Y h:i:sa"),
								);
						$docid=	$this->credit_manager_user_model->saveDocuments($file_input_arr);
						}
						$json = array (  
								'docid'=>$docid,
									'status'=>"success",
			...
			public function save_KYC_document()
			{
						...
						if(!empty($_FILES['file']['name']))
				{
						...
						exec($correcturl);
								...
								'doc_cloud_name' => $filename,
								'doc_cloud_dir' => $dirname,
									'Doc_updated_by'=>$data['dsa_id'],
								'Doc_updated_date'=>date('d-m-Y'),
								'Doc_uploded_type'=>"Post Sanction Additional Document"
							);
							$docid = $this->Customercrud_model->saveDocuments($file_input_arr);
				...
				public function save_Property_document()
			{
				$data=array(
                    'loan_application_id'=>$this->input->post('loan_application_id'),
                    'customer_id'=>$this->input->post('customer_id'),
					'dsa_id'=>$this->input->post('dsa_id'),
					'select_property_doc_name'=>$this->input->post('select_property_doc_name'),
		            );
						$id=$this->input->post('customer_id');
					//print_r($data);
						...
						if(!empty($_FILES['file']['name']))
				{
						...
						$unique_code=$this->input->post('customer_id');
							...
							$Doc_type = "KYC";
							$dirname = "Finserv/Property_documents/".$id;
						...
						exec($correcturl);
								 ...
								 $file_input_arr = array(
								'USER_ID' => $id,
								'DOC_TYPE' =>$data['select_property_doc_name'],	
								'DOC_NAME' => $filename,
								'DOC_SIZE' => $_FILES['file']['size'],
								'DOC_FILE_TYPE' => $_FILES['file']['type'],
								'DOC_MASTER_TYPE' => 'Property Document',
								'doc_cloud_name' => $filename,
								'doc_cloud_dir' => $dirname,
									'Doc_updated_by'=>$data['dsa_id'],
								'Doc_updated_date'=>date('d-m-Y'),
			...
			public function save_income_document()
			{
				$data=array(
                    'loan_application_id'=>$this->input->post('loan_application_id'),
                    'customer_id'=>$this->input->post('customer_id'),
					'dsa_id'=>$this->input->post('dsa_id'),
					'select_income_doc_name'=>$this->input->post('select_income_doc_name'),
		            );
						
						$id=$this->input->post('select_income_user_name');
					//print_r($data);
					
						if(!empty($_FILES['file']['name']))
				{
						$Application_id = $this->input->post('loan_application_id');
						$unique_code=$this->input->post('select_income_user_name');
						
				
						
							$Doc_type = "KYC";
							$dirname = "Finserv/Income_documents/".$id;
						...
						exec($correcturl);
								 ...
								 $file_input_arr = array(
								'USER_ID' => $id,
								'DOC_TYPE' =>$data['select_income_doc_name'],	
								'DOC_NAME' => $filename,
								'DOC_SIZE' => $_FILES['file']['size'],
								'DOC_FILE_TYPE' => $_FILES['file']['type'],
								'DOC_MASTER_TYPE' => 'INCOME PROOF',
								'doc_cloud_name' => $filename,
								'doc_cloud_dir' => $dirname,
								  'Doc_updated_by'=>$data['dsa_id'],
								'Doc_updated_date'=>date('d-m-Y'),
			...
			public function post_disbursement_documents()
			{
				$data=array(
                    'customer_id'=>$this->input->post('customer_id'),
					'select_post_document_name'=>$this->input->post('select_post_document_name'),
		            );
				...
				if(!empty($_FILES['file']['name']))
				{
							...
							$dirname = "Finserv/Post_Disbursement_Documents/".$id;
						...
						exec($correcturl);
							     ...
							     $file_input_arr = array(
								'USER_ID' => $id,
								'DOC_TYPE' =>$this->input->post('select_post_document_name'),	
								'DOC_NAME' => $filename,
								'DOC_SIZE' => $_FILES['file']['size'],
								'DOC_FILE_TYPE' => $_FILES['file']['type'],
								'DOC_MASTER_TYPE' => 'Post Sanctioned Documents',
								'doc_cloud_name' => $filename,
								'doc_cloud_dir' => $dirname
							);
							$docid = $this->Customercrud_model->saveDocuments($file_input_arr);
			...
			public function save_credit_proceed_status()
			{
				   $data=array(
								'customer_id'=>$this->input->post('customer_id'), //
								'dsa_id'=>$this->input->post('dsa_id'), 
				   ...
				   $file_input_arr = 
				            array(
								'Credit_proceed_status' => $data['status'],
								'Credit_proceed_status_updated_on' =>date("d-m-Y"),
								'Credit_proceed_status_updated_by' => $data['dsa_id'],
					   		  );
					
					
					//======================== send email to disbursement ops======================================================
					    $credit_manager_info= $this->Operations_user_model->getProfileData($data['dsa_id']);
						$customer_info = $this->Operations_user_model->getProfileData($data['customer_id']);
						$getCustomerSanction_letter_details=  $this->credit_manager_user_model->getCustomerSanction_letter_details($data['customer_id']);
						$data['loan_details']=$this->Operations_user_model->getAplliedLoanDetails($data['customer_id']);
						$customer_name =$customer_info->FN." ".$customer_info->LN;
						 ...
						 //   $send_email_to_3_HR="operations@finaleap.com,credit@finaleap.com,accountsfin@finaleap.com";
						    $send_email_to_3_HR='operations@finaleap.com';
						    $this->email->to($send_email_to_3_HR);
							 $this->email->cc('credit@finaleap.com,accountsfin@finaleap.com');
						    $this->email->bcc('info@finaleap.com');
							$mail_data['confirmed_by']= $credit_manager_info->FN." ".$credit_manager_info->LN;
							$mail_data['customer_name']=$customer_name;
							$mail_data['Application_id']=$data['loan_details']->Application_id;
						...
						$this->email->subject('Disbursement Confirmation For the customer :  '. $customer_name .''); 
						$mailContent = $this->load->view('templates/Disbursement_confirmation_form_credit',$mail_data,true);
						$this->email->message($mailContent); 
						if($this->email->Send())
						{
							$docid = $this->credit_manager_user_model->update_sanction_letter_details($data['customer_id'],$file_input_arr);
								$json = array (  
								'status'=>"success",
									  );
								echo json_encode($json);
				...
				//========================================= added by priya 12-11-2022
			public function save_sanction_conditions_details()
			{
				   $data=array(
								'customer_id'=>$this->input->post('customer_id'), //
								'dsa_id'=>$this->input->post('dsa_id'), 
								'select_sanction_condition_name'=>$this->input->post('select_sanction_condition_name'),
								'is_otc'=>$this->input->post('is_otc'), 
								'is_pdd'=>$this->input->post('is_pdd'), 
								'not_applicable'=>$this->input->post('not_applicable'), 
								'otc_date'=>$this->input->post('otc_date'), 
					...
					if(!empty($_FILES['file1']['name']))
				{
							...
							$dirname = "Finserv/SanctionCondition_Documents/".$id;
												
					    $curlurl = "curl -X MKCOL -u ".CLOUD_USERNAME.":".CLOUD_PASSWORD." https://files.finaleap.com/remote.php/dav/files/".CLOUD_PATH."/".$dirname."";
						exec($curlurl);
						$time = time();
						$fileextensionarr = explode(".",$_FILES["file1"]["name"]);
						$fileextension = $fileextensionarr[1];
						$filename = $time.".".$fileextension;
						$dirname = "Finserv/SanctionCondition_Documents/".$id."/";
							
						$filenamewithdir = $dirname.$filename;
						$correcturl = "curl -X PUT -u ".CLOUD_USERNAME.":".CLOUD_PASSWORD." https://files.finaleap.com/remote.php/dav/files/".CLOUD_PATH."/".$filenamewithdir." -T ".$_FILES["file1"]["tmp_name"]."";
						exec($correcturl);
							}
							else{
								 $filename = "";
					...
					if(!empty($_FILES['file2']['name']))
				           {
							...
							$dirname2 = "Finserv/SanctionCondition_OTCPDD/".$id;
												
					    $curlurl = "curl -X MKCOL -u ".CLOUD_USERNAME.":".CLOUD_PASSWORD." https://files.finaleap.com/remote.php/dav/files/".CLOUD_PATH."/".$dirname2."";
						exec($curlurl);
						$time = time();
						$fileextensionarr = explode(".",$_FILES["file2"]["name"]);
						$fileextension = $fileextensionarr[1];
						$filename2 = $time.".".$fileextension;
						$dirname2 = "Finserv/SanctionCondition_OTCPDD/".$id."/";
							
						$filenamewithdir = $dirname2.$filename2;
						$correcturl = "curl -X PUT -u ".CLOUD_USERNAME.":".CLOUD_PASSWORD." https://files.finaleap.com/remote.php/dav/files/".CLOUD_PATH."/".$filenamewithdir." -T ".$_FILES["file2"]["tmp_name"]."";
						exec($correcturl);
							}
							else{
								 $filename2 = "";
								 $dirname2=""; 
						 	     ...
						 	     $file_input_arr = array(
						        'Customer_id' =>$this->input->post('customer_id'),	
								'SanctionCondition' =>$this->input->post('select_sanction_condition_name'),	
								'Documnt_name' => $filename,
								'OTC' => $data['is_otc'],
								'PDD' => $data['is_pdd'],
								'Not_applicable' => $data['not_applicable'],
								'updated_BY' => $data['dsa_id'],
								'updated_on' => date('d-m-Y'),
								'Document_path' => $dirname,
								'OTC_Date' =>$data['otc_date'],
								'OTC_PDD_document' => $filename2 ,
								'OTC_PDD_document_path' => $dirname2,
							);
							//echo "<pre>";
							//print_r( $file_input_arr);
							//exit();
							$condition =$file_input_arr['SanctionCondition'];
					
							$check_existing = $this->CustomerCrud_model->checkSanctionCondition($file_input_arr['Customer_id'],$condition);
						    if(!empty($check_existing))
							{
								$docid = $this->Customercrud_model->UpdateSanctionConditionDocuments($file_input_arr['Customer_id'],$condition ,$file_input_arr);
							}
							else
							{
								$docid = $this->Customercrud_model->saveSanctionConditionDocuments($file_input_arr);
							}
		...
		public function show_info_for_sanction_condition()
			{
				   $data=array(
								'customer_id'=>$this->input->post('customer_id'), //
								'sanction_condition_for_info'=>$this->input->post('sanction_condition_for_info'), 
											
							  );
					$check_existing = $this->CustomerCrud_model->checkSanctionCondition($data['customer_id'],$data['sanction_condition_for_info']);
					
					 $json = array (  
								'status'=>"success",
								'ID'=>$check_existing->ID,
								'SanctionCondition'=>$check_existing->SanctionCondition,
								'Document_path'=>$check_existing->Document_path,
								'Documnt_name'=>$check_existing->Documnt_name,
								'OTC'=>$check_existing->OTC,
								'PDD'=>$check_existing->PDD,
								'Not_applicable'=>$check_existing->Not_applicable,
								'updated_BY'=>$check_existing->updated_BY,
								'updated_on'=>$check_existing->updated_on,
								'OTC_Date'=>$check_existing->OTC_Date,
								'OTC_PDD_document'=>$check_existing->OTC_PDD_document,
								'OTC_PDD_document_path'=>$check_existing->OTC_PDD_document_path,
									  );
								echo json_encode($json);
			}
				...
				public function save_remarks_queries()
			{
				   ...
				   $getCustomerSanction_letter_details=	$this->credit_manager_user_model->getCustomerSanction_letter_details($data['customer_id']);
                   $current_data=$getCustomerSanction_letter_details->disbursement_queries;
				  // echo "<pre>";
				  // print_r(json_decode($current_data,true));
				   ...
				   if( $login_user_info->ROLE == 26)
				   {
					      $file_input_arr = 
				            array(
								'disbursement_queries' => $final_data,
								'Credit_proceed_status'=>'Reverted'
								  );
				   }
				   else 
				   {
	...
	//=================================================================9-12-2022------------------ added by priya ============================================= -->
	 public function DR(){
		...
		if($this->session->userdata('USER_TYPE') == '' || $this->session->userdata('USER_TYPE') == 'CUSTOMER'){
			...
			$data['row']=$this->Customercrud_model->getProfileData($id);
		    $this->load->view('dashboard_header', $data);
		    $this->load->view('credit_manager_user/Disbursement_reverted_files', $arr);

		
		}
	}
	
	public function Disbursement_reverted_files_PG()
{
	$draw = $_POST['draw'];
	$row = $_POST['start'];
	$rowperpage = $_POST['length']; 
	...
	//exit();
    $pass_array=array_unique($Uniquecode_array );
	$sel = $this->credit_manager_user_model->getReverted_doc_count();
	$totalRecords =$sel;
	$sel=$this->credit_manager_user_model->searchReverted_files($searchValue);
	$totalRecordwithFilter =$sel;
	$sel=$this->credit_manager_user_model->getreverted_files_With_Page($searchValue,$columnName,$columnSortOrder,$row,$rowperpage);
	$empRecords = $sel;
	...
	/*---------------------------------------------- added by priya 04-01-2023------------------------------------------------------ */
		public function save_legal_conditions_details()
			{
				   $data=array(
								'customer_id'=>$this->input->post('customer_id'), //
								'dsa_id'=>$this->input->post('dsa_id'), 
								'select_legal_condition_name'=>$this->input->post('select_legal_condition_name'),
								'is_otc'=>$this->input->post('is_otc'), 
								'is_pdd'=>$this->input->post('is_pdd'), 
								'otc_date'=>$this->input->post('otc_date'), 
					...
					if(!empty($_FILES['file1']['name']))
				{
							...
							$dirname = "Finserv/LegalCondition_Documents/".$id;
					...
					if(!empty($_FILES['file2']['name']))
				           {
							...
							$dirname2 = "Finserv/LegalCondition_OTCPDD/".$id;
						...
						exec($correcturl);
								 ...
								 $dirname2=""; 
						 	     ...
						 	     $file_input_arr = array(
						        'Customer_id' =>$this->input->post('customer_id'),	
								'LegalCondition' =>$this->input->post('select_legal_condition_name'),	
								'Documnt_name' => $filename,
								'OTC' => $data['is_otc'],
								'PDD' => $data['is_pdd'],
								'updated_BY' => $data['dsa_id'],
							...
							//print_r( $file_input_arr);
							//exit();
							$condition =$file_input_arr['LegalCondition'];
					
							$check_existing = $this->credit_manager_user_model->checkLegalCondition($file_input_arr['Customer_id'],$condition);
						    if(!empty($check_existing))
							{
								$docid = $this->credit_manager_user_model->UpdateLegalConditionDocuments($file_input_arr['Customer_id'],$condition ,$file_input_arr);
							}
							else
							{
								$docid = $this->credit_manager_user_model->saveLegalConditionDocuments($file_input_arr);
							}
		...
		public function show_info_for_legal_condition()
			{
				   $data=array(
								'customer_id'=>$this->input->post('customer_id'), //
								'legal_condition_for_info'=>$this->input->post('legal_condition_for_info'), 
											
							  );
					$check_existing = $this->credit_manager_user_model->checkLegalCondition($data['customer_id'],$data['legal_condition_for_info']);
					
					 $json = array (  
								'status'=>"success",
								'ID'=>$check_existing->ID,
								'LegalCondition'=>$check_existing->LegalCondition,
								'Document_path'=>$check_existing->Document_path,
								'Documnt_name'=>$check_existing->Documnt_name,
								'OTC'=>$check_existing->OTC,
								'PDD'=>$check_existing->PDD,
		...
		/*--------------------------------------- addded by papiha on 09_01_2023---------------------------------------------------*/
		public function Vendors_Roports()
		{
             $Application_ID=base64_decode($this->input->get('x'));
			 $cust_info=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_ID);
			 $data['cust_info']=$cust_info;
			 $id=$cust_info->UNIQUE_CODE;
			 $uid = $this->session->userdata('ID');
			 $data['row']=$this->Customercrud_model->getProfileData($uid);
			 $age = $this->session->userdata('AGE');
			 $data['showNav'] = 1;
			 $data['age'] = $age;
			 $data['Legal_docs']=$this->Legal_model->get_Legal_docs($Application_ID);
			 $data['Technical_docs']=$this->Legal_model->get_Technical_docs($Application_ID);
			 $data['FI_docs']=$this->Legal_model->get_fi_docs($Application_ID);
			 $data['RCU_docs']=$this->Legal_model->get_RCU_docs($Application_ID);
			 $data['userType'] = $this->session->userdata("USER_TYPE");
		     $data['find_my_legal_documents']=$this->credit_manager_user_model->find_my_legal_documents($id);
			 $data['find_my_technical_documents']=$this->credit_manager_user_model->find_my_technical_documents($id);
			 $data['find_my_FI_documents']=$this->credit_manager_user_model->find_my_FI_documents($id);
			 $data['find_my_RCU_documents']=$this->credit_manager_user_model->find_my_RCU_documents($id);
			 $data['find_my_Court_case_documents']=$this->credit_manager_user_model->find_my_Court_case_documents($id);
			 $data['find_my_legal_vetting_documents']=$this->credit_manager_user_model->find_my_legal_vetting_documents($id);
			 $data['find_my_Legal_Document_Search_documents']=$this->credit_manager_user_model->find_my_Legal_Document_Search_documents($id);
			 $this->load->view('dashboard_header', $data);
			 $this->load->view('credit_manager_user/vendor_reports', $data);
