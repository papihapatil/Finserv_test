 ...
 if(!empty($empRecords)){
    ...
    foreach($empRecords as $row)
    {
		...
		$data[] = array(
         //"ID"=>$row->ID,
        "FN"=>$row->FN.' '.$row->LN,
		"Existing"=>'<a href="'.base_url().'index.php/admin/Loannew?id='.base64_encode($row->UNIQUE_CODE).'">'.$Existing.'</a>',
        "New"=>'<a href="'.base_url().'index.php/admin/Loannew?id='.base64_encode($row->UNIQUE_CODE).'">'.$New.'</a>',
		"Customer_existing"=>'<a href="'.base_url().'index.php/BranchManager/BM_Existing_Today_souring?id='.base64_encode($row->UNIQUE_CODE).'">'.$Customer_existing.'</a>',
		"Customer_ntb"=>'<a href="'.base_url().'index.php/BranchManager/BM_NTB_Today_souring?id='.base64_encode($row->UNIQUE_CODE).'">'.$Customer_ntb.'</a>',
    );                                                                 
     }
 }
	   else{
...
//exit; 
 ...
 public function Telecaller()
	...
	public Function Download_LMS_Excel()
	{					
				//echo "hiiii";
				$this->load->library('excel');
				$this->excel->setActiveSheetIndex(0);
