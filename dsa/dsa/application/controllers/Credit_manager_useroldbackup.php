...
class Credit_manager_user extends CI_Controller {
	...
	public function customers(){
		...
		if($this->session->userdata('USER_TYPE') == '' || $this->session->userdata('USER_TYPE') == 'CUSTOMER'){
			...
			if($this->session->userdata('USER_TYPE') =='credit_manager_user')$this->load->view('credit_manager_user/customers_new_PG', $arr);
...
//==============================================added by priyanka==============================================
  ...
  exit();
	...
	//===============================function for form submission==============================
	 ...
	 { // here application_number is same as CUST_ID in another fields
	    //$application_number     =$this->session->userdata("id"); // shows customer id
		 $credit_manager_user_id =$this->session->userdata("id1");
	   //$application_number2=$this->session->userdata("id");
	   //echo $this->input->post('loan_status');
	   //exit();
	   ...
	   $date = date('d-m-y');
		...
		//exit();
		//echo $tenure;	       echo $loan_type;				echo $roi; 	 //  echo $loan_amount;	        echo $channel;	       echo $product;				echo $program;
	  // echo $income_assessed;	    echo $final_tenure;	   echo $eligibility;		    echo $approval_remark;	  // echo $final_loan_amount;	    echo $recommended_by;	 echo $credit_manager_user_id;  echo $application_number;	 exit();
		 $data = array(
		...
		if(isset($check_existing_entry))
		{
			...
			$data=array(
			  'credit_manager_id' =>$credit_manager_user_id,
			  'UNIQUE_CODE' =>$application_number,
			  'credit_sanction_status'=>$status,
			  'loan_status'=>$loan_status
			
			);
			$result_for_status = $this->credit_manager_user_model->update_credit_sanction_status($application_number,$data);
			$this->session->set_flashdata('success', 'Credit sanction Updated successfully');
		    redirect("credit_manager_user/credit_sanction_loan_first_level?ID=$application_number");
		...
		else
		{
			...
			$this->session->set_flashdata('success', 'Credit sanction done successfully');
		    redirect("credit_manager_user/credit_sanction_loan_first_level?ID=$application_number");
		}
...
public function dsa()
 {
	...
	public function Send_TO_Legal()
	{
		...
		public function customers_pd()
		{
			if($this->session->userdata('USER_TYPE') == '' || $this->session->userdata('USER_TYPE') == 'CUSTOMER'){
				...
				$data['row']=$this->Customercrud_model->getProfileData($id);
				$this->load->view('dashboard_header', $data);
				if($this->session->userdata('USER_TYPE') =='credit_manager_user')$this->load->view('credit_manager_user/customers_for_pd', $arr);
				else if($this->session->userdata('USER_TYPE') == 'DSA')$this->load->view('dsa/customers_new', $arr);
				else if($this->session->userdata('USER_TYPE') == 'DSA_MANAGER')$this->load->view('manager/customers_new', $arr);
				else if($this->session->userdata('USER_TYPE') == 'DSA_CSR')$this->load->view('csr/customers_new', $arr);
	...
	public function Vendors()
	{
		...
		$decodede_id=base64_decode($this->input->get('x'));
		 ...
		 $this->load->helper('url');
                ...
                $data['age'] = $age;
                $data['userType'] = $this->session->userdata("USER_TYPE");
			    $data_row = $this->credit_manager_user_model->getDocuments_send_to_legal($Application_ID,$data['userType'],'Legal');
	...
	function do_upload_Legal_doc()
	{
		if(isset($_POST['upload']))
		{
			...
			$chk_res = $this->credit_manager_user_model->checkSavedDocument($Application_id, $Doc_type);
	        if($chk_res > 0)
			{
				
				$error = "Document ".$Doc_type." already saved by you.";
				redirect('credit_manager_user/Vendors?x='.base64_encode($Application_id).'&error='.$error, 'refresh');
			}
			else
			{
			    for($i=0;$i<$count;$i++)
		...
		if(isset($_POST['submit']))
		{
			...
			$data_doc = array(
							...
							'Legal_Status'=>'Send_TO_Legal',
							'Legal_id'=>$Legal_id,
							'Legal_name'=>$Legal_name,
							'Credit_id'=>$uid,
							'Send_to_legal_date'=>date("d-m-Y h:i:sa"),
							'Last_updated_by_credit'=>date("d-m-Y h:i:sa")
			...
			$result_docs_bank_id= $this->CustomerCrud_model->update_customer_applied_loan($cust_info->UNIQUE_CODE,$data_bank_id);
			$result_docs1 = $this->credit_manager_user_model->Update_Legal_status($cust_info->UNIQUE_CODE,$data_status);
			$check_id=$this->Legal_model->get_legal_docs($Application_id);
			if($check_id>0)
			{
				 $result_docs = $this->credit_manager_user_model->Update_Legal_Documents($Application_id,$data_doc);
			...
			$notification_data=['user_id'=>$this->input->post('Legal'),'notification'=>'New Customer is Assigned To You :'.$this->input->post('name'),'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data); 
			if($result_docs>0)
			{
				/*-----------------------------send Mail to Legal------------------------------*/
			...
			$send_to_emails= implode(",",$Eamils);	
			$send_to_emails=$send_to_emails.'papiha.patil@finaleap.com,sachin.kardile@finaleap.com,arun.pawar@finaleap.com,sandeep.belbhandare@finaleap.com,prashant.kolhapure@finaleap.com';
				$this->sendEmail_all_sent_to_legal($user_info,$cust_info, $send_to_emails,$Application_id);
				...
				$config['protocol']='smtp';
					...
					$from_email = "infoflfinserv@finaleap.com";
					$this->email->from($from_email,'Finaleap'); 
				    $send_email_to_support=$user_info->EMAIL;
					//$send_email_to_support ='priyanka.abdagire@finaleap.com';
					$this->email->to($send_email_to_support);
					$data['cust_info']=$cust_info;
					$data['Legal_info']=$user_info;
					//$this->email->subject('New Customer Assigned To You For Legal Process: '.$cust_info->FN.' '.$cust_info->LN); 
					$this->email->subject($Application_id.'----'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'---- LEGAL REPORT INITIATE'); 
					$mailContent = $this->load->view('templates/Legal_mail_content_updated',$data,true);
					//$mailContent='hiii';
		 ...
		 else if(isset($_POST['revert_action']))
		{
			...
			$notification_data=['user_id'=>$this->input->post('Legal'),'notification'=>'Revert Action Taken For Customer :'.$this->input->post('name'),'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data);
			/*------------------------------------------send mail to Legal------------------------------------*/
			/*------------------------------Send mail to Technical---------------------------------*/
          $user_info= $this->Operations_user_model->getProfileData($Legal_id);
			 ...
			 table,td,th{
				 ...
				 body {
					font-family: Arial, sans-serif;
							  ...
							  <p style="font-size:12px;">Please process further & submit the final report as early as possible.</p>';
						  ...
						  $msg.='
				                   ...
				                   <p style="font-size:12px;">Finaleap.Pvt.Ltd </p>
								  </body>
								  </html>';
		  ...
		  //	$this->email->message('Dear Team ,'."\r\n\n".'Following application is accepted by - '.$HR_name."\r\n".'Name		:'.$name."\r\n".'Applied for	:'.$applied_for."\r\n".'Email ID	:'.$email."\r\n".'Mobile		:'.$mobile."\r\n".'Total Experience	:'.$total_experience."\r\n".'Applied on		:'.$date."\r\n\n\n".'Best Regards,'."\r\n".'Finaleap Private Limited '); 
			...
			if($cust_info->Location!=NULL)
			{
				...
				/*foreach($cpas as $cpa)
				{
				 ...
				 array_push($Eamils,'papiha.patil@finaleap.com');
			     $send_to_emails= implode(",",$Eamils);
				$this->customer_revert_action_to_all($user_info,$cust_info, $send_to_emails,$Application_id);	*/
		...
		else if(isset($_POST['revert_to_legal']))
		{
			$Application_id = $this->input->post('Application_Id');
			$cust_info=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_id);
			$uid = $this->session->userdata('ID');
			...
			$notification_data=['user_id'=>$this->input->post('Legal'),'notification'=>'Revert  For Customer :'.$this->input->post('name'),'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data); 
			...
			$send_to_emails= implode(",",$Eamils);	
			$send_to_emails=$send_to_emails.'papiha.patil@finaleap.com,sachin.kardile@finaleap.com,arun.pawar@finaleap.com,sandeep.belbhandare@finaleap.com,prashant.kolhapure@finaleap.com';
			$this->customer_revert_from_finaleap($user_info,$cust_info, $send_to_emails,$Application_id,$Credit_Comment);
            $this->customer_revert_from_finaleap_Team($user_info,$cust_info, $send_to_emails,$Application_id,$Credit_Comment);			
			/*---------------------------- send mail--------------------------------------------*/
			if($result_docs>0)
			{
				$this->session->set_userdata("result", 1);
		...
		else if(isset($_POST['Accept']))
		{
			...
			$data_doc = array(
							...
							'Application_id'=>$Application_id,
							'Credit_Comment_Legal'=>$Credit_Comment,
							'Legal_Status'=>'Accept From Finaleap',
							'Legal_id'=>$Legal_id,
							'Legal_name'=>$Legal_name,
							'Credit_id'=>$uid,
							'Last_updated_by_credit'=>date("d-m-Y h:i:sa") 
			...
			$notification_data=['user_id'=>$this->input->post('Legal'),'notification'=>'Accept Report For Customer :'.$this->input->post('name'),'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data);
             $user_info= $this->Operations_user_model->getProfileData($Legal_id);			
             /*--------------------------------	send mail to Legal-----------------------------------*/
          $Appication_info=$this->Legal_model->get_legal_docs($Application_id);
		  ...
		  //	$this->email->message('Dear Team ,'."\r\n\n".'Following application is accepted by - '.$HR_name."\r\n".'Name		:'.$name."\r\n".'Applied for	:'.$applied_for."\r\n".'Email ID	:'.$email."\r\n".'Mobile		:'.$mobile."\r\n".'Total Experience	:'.$total_experience."\r\n".'Applied on		:'.$date."\r\n\n\n".'Best Regards,'."\r\n".'Finaleap Private Limited '); 
			...
			$send_to_emails= implode(",",$Eamils);	
			$send_to_emails=$send_to_emails.'papiha.patil@finaleap.com,sachin.kardile@finaleap.com,arun.pawar@finaleap.com,sandeep.belbhandare@finaleap.com,prashant.kolhapure@finaleap.com';
			//$this->Accept_report($user_info,$cust_info, $send_to_emails,$Application_id,$Credit_Comment);
            $this->Accept_report_Team($user_info,$cust_info, $send_to_emails,$Application_id,$Credit_Comment);			
			/*---------------------------- send mail--------------------------------------------*/
			if($result_docs>0)
			{
				$this->session->set_userdata("result", 1);
...
function do_upload_Technical_doc()
	{
			    ...
			    for($i=0;$i<$count;$i++)
						 {
							...
							//$file_size = $_FILES["userfile"]['size'][$i];
							  ...
							  $_FILES['file']['size'] = $_FILES['userfile']['size'][$i];
								$config = array(
									'upload_path' => "./images/Technical_Documents/".$Application_id,
									'allowed_types' => "gif|jpg|png|jpeg|pdf",
									'overwrite' => TRUE
								);
							...
							$file_input_arr = array(
								'Application_id' => $Application_id,
								'Doc_name' => $Doc_type,	
								'path' =>"./images/Technical_Documents/".$Application_id."/".$_FILES['file']['name'],
								'User'=>$u_type,
								'Send_to'=>$this->input->post('send_to')
								
							);
							
							if (!file_exists('./images/Technical_Documents/'.$Application_id) ){   
								 mkdir('./images/Technical_Documents/'.$Application_id);
									if($this->upload->do_upload('file'))
									{
										 //$this->session->set_userdata("result", 1);
										 $this->credit_manager_user_model->saveDocuments($file_input_arr);
		...
		if(isset($_POST['submit']))
		{
			...
			$cust_info=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_id);
			$uid = $this->session->userdata('ID');
			$Technical_id=$this->input->post('Technical');
			$Technical_name=$this->input->post('Technical_name');
			$Credit_Comment=$this->input->post('comment');
			$form_id = $this->input->post('form_id');
			$bank_name= $this->input->post('bank_name');
			$data_doc = array(
							...
							'Technical_Status'=>'Send_TO_Technical',
			...
			$notification_data=['user_id'=>$this->input->post('Technical'),'notification'=>'New Customer is Assigned To You :'.$this->input->post('name'),'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data); 
			if($result_docs>0)
			{
				/*-----------------------------send Mail to Legal------------------------------*/
				$user_info= $this->Operations_user_model->getProfileData($Technical_id);
				if($cust_info->Location !=NULL)
				{
				$get_credit=$this->Operations_user_model->getcredit_by_location($cust_info->Location);
				$get_cluster=$this->Operations_user_model->getcluster_by_location($cust_info->Location);
			...
			$send_to_emails= implode(",",$Eamils);	
			$send_to_emails=$send_to_emails.'papiha.patil@finaleap.com,sachin.kardile@finaleap.com,arun.pawar@finaleap.com,sandeep.belbhandare@finaleap.com,prashant.kolhapure@finaleap.com';
				$this->sendEmail_all_sent_to_Technical($user_info,$cust_info, $send_to_emails,$Application_id);
				...
				$config['protocol']='smtp';
					...
					$mailContent = $this->load->view('templates/Technical_mail_content_updated',$data,true);
					//$mailContent='hiii';
					$this->email->message($mailContent); 
					$this->email->Send();
				$this->session->set_userdata("technical", 1);
				redirect('Credit_manager_user/Vendors?x='.base64_encode($Application_id));
			}
			else
			{
				$this->session->set_userdata("technical", 0);
				redirect('Credit_manager_user/Vendors?x='.base64_encode($Application_id));
			}
		}
		 else if(isset($_POST['revert_action']))
		{
			...
			$cust_info=$this->credit_manager_user_model->getCustomers_for_pd_info($Application_id);
			$uid = $this->session->userdata('ID');
			$Techical_id=$this->input->post('Technical');
			$Techical_name=$this->input->post('Techical_name');
			$Credit_Comment=$this->input->post('comment');
			$form_id = $this->input->post('form_id');
			$data_doc = array(
							...
							'Technical_id'=>$Techical_id,
							'Technical_name'=>$Techical_name,
			...
			$notification_data=['user_id'=>$this->input->post('Technical'),'notification'=>'Revert Action Taken For Customer :'.$this->input->post('name'),'status'=>'unseen'];
			$notification=$this->Admin_model->insert_notification($notification_data);
            /*------------------------------Send mail to Technical---------------------------------*/
          $user_info= $this->Operations_user_model->getProfileData($Techical_id);
			 ...
			 table,td,th{
				 ...
				 body {
					font-family: Arial, sans-serif;
							  ...
							  <body>
							  <p style="font-size:12px;">Dear Sir / Mam,</p>
							  <p style="font-size:12px;">This is to inform you that  queries raised on the Legal Report of Mr/Mrs. <b>'.strtoupper($cust_info->FN).' '.strtoupper($cust_info->LN).'</b>has been successfully resolved.</b> </p>
							  <p style="font-size:12px;">Please process further & submit the final report as early as possible.</p>';
