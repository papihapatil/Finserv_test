<?php
    ...
    if (isset($message)) {
		if($message==1)
		{
        echo '<script> swal("success!", "Send To RCU Successfully", "success");</script>';
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
											...
											<div class="container">
												...
												<h3>Send Customer To RCU Process</h3>
												</div>
												
												<br>
												</br>
												<Form action="<?php echo base_url('index.php/Credit_manager_user/do_upload_RCU_doc');?>" method="post" enctype="multipart/form-data" id="Legal_doc_upload">
												<div class="row">
												   <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
													<input type="number" name="form_id" value="3" hidden />
													<input type="text" name="send_to" value="RCU" hidden />
												...
												<?php  foreach($documents as $row){?>
													...
													<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
														<div class="row " style="border: 1px solid #edeff1; margin-left:13px; margin-right:13px">
															...
															<div class="col-sm-1"> 	
															<a style="margin-left: 8px; " href="<?php echo base_url();?>index.php/customer/delete_vendor_doc_RCU/<?php echo $row->id; ?>"><i class="fa fa-trash text-right" style="color:red;"></i></a>
												...
												<Form action="<?php echo base_url('index.php/Credit_manager_user/do_upload_RCU_doc');?>" method="post" enctype="multipart/form-data">
												<div class="row">
												...
												<div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12">
													 <div style=" margin-top: 8px;" class="col">
														 <label for="email" style="font-size: 16px; color: black; font-weight: bold;"> Assign To RCU</label>
													<?php if(!empty($Legal_docs)){if($Legal_docs->RCU_status!='')
													{?>
													<select required class="form-control select" name="RCU_name" id="RCU_name" required> 
													<option value="<?php if(!empty($Legal_docs)){ echo $Legal_docs->RCU_id; }?>" selected ><?php if(!empty($cust_info))echo $Legal_docs->RCU_name;?></option>
													</select>
													  <input type="text" id="RCU_name_1" name="RCU_name_1"  value="<?php if(!empty($cust_info))echo $cust_info->Legal_name; ?>" hidden >
													<?php }else{?>
														<select required class="form-control" name="RCU_name" id="RCU_name" required > 
													<option value="" > select RCU </option>
												...
												<?php if(!empty($Legal_docs)){if($Legal_docs->RCU_status=='Revert From RCU'){?>
													<div class="row">
														<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
														<div style=" margin-top: 8px;" class="col">
															 <label for="email" style="font-size: 16px; color: black; font-weight: bold;"> RCU Comment </label>
															<textarea name="comment_RCU" class="form-control"><?php echo $Legal_docs->RCU_comment; ?> </textarea>
												...
												<?php if(!empty($Legal_docs)){if($Legal_docs->RCU_status=='Document Received From RCU' || $Legal_docs->RCU_status=='Accepted from Finaleap' )
													{ ?>
												<div class="text-center">
												<h3>RCU Report</h3>
												...
												<?php if(isset($Legal_docs))
																   {
																	   ...
																	   $Conditions_to_Loan_Sanction_2= json_decode($Legal_docs->additional_comments);
																				   ...
																				   <div class="row">
																						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
																						    <div style=" margin-top: 8px;" class="col">
																							<label for="email" style="font-size: 16px; color: black; font-weight: bold;"> Comments From RCU </label>
												...
												<?php }
												else if($Legal_docs->RCU_status=='Document Received From RCU')
														{?>
															<button type="submit" name="revert_to_RCU" class="btn btn-primary" id='revert_to_RCU' >Revert To RCU</button>
														    <button type="submit" name="Accepted" class="btn btn-primary" id='Accepted' >Accepted</button>
													<?php	}
													else if($Legal_docs->RCU_status=='Send TO RCU')
														{?>
                                                                <button type="submit" name="revert_action " class="btn btn-primary disabled" >Wating For RCU Reports</button>
														<?php 
														}
														else if($Legal_docs->RCU_status=='Revert Action Taken')
														{?>
                                                                <button type="submit" name="revert_action " class="btn btn-primary disabled" >Revert Action Taken</button>
														<?php
														}
														else if($Legal_docs->RCU_status=='Accepted from Finaleap')
														{?>
                                                                <button type="submit" name="revert_action " class="btn btn-primary disabled" >Accepted from Finaleap</button>
														<?php
														}
														else if($Legal_docs->RCU_status=='Revert From Finaleap')
														{?>
...
$(document).ready(function () {
				...
				$.ajax({
				 ...
				 success:function(data){
									 ...
									 if(obj.RCU_required=='yes')
									 {
										 ...
										 var url = window.location.origin+"/finserv_test/dsa/dsa/index.php/Legal/get_RCU_by_bank";
											$.ajax({
												 type:'POST',
												 url:url,
	...
	$( "#revert_to_RCU" ).click(function(e) {
									...
									var content='<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">'+
														'<div class="row " style="border: 1px solid #edeff1; margin-left:13px; margin-right:13px">'+
															'<div class="col-sm-10"><a href="'+value.path+'" target="_blank">  <i class="fa fa-file-text" style="color:green" aria-hidden="true"></i> &nbsp; <label for="email" style="font-size: 13px; color: black; font-weight: bold;">'+ value.Doc_name +'</label></a></div>'+
																...
																'<div class="col-sm-1">'+	
															       '<a style="margin-left: 8px; " href="'+window.location.origin+'/finserv_test/dsa/dsa/index.php/customer/delete_vendor_doc_RCU/'+value.id+'"><i class="fa fa-trash text-right" style="color:red;"></i></a>'+
															'</div>'+			
														'</div></div></div>';
