			...
			if($this->session->userdata("USER_TYPE") == ''){
				...
				$End_Date  = date('Y-m-t');
				 ...
				 $data['End_Date'] = $End_Date;
				  ...
				  $dashboard_data = $this->Admin_model->current_month2($Start_Date,$End_Date,$id);
				 ...
				 $dashboard = $this->Admin_model->current_month2($Curr_Date,$Today_Date,$id);
 ...
 public function Lead_souring(){
    ...
    if($this->session->userdata('USER_TYPE') == '' || $this->session->userdata('USER_TYPE') == 'CUSTOMER' || $this->session->userdata('SITE') != 'finserv'){
       ...
       // $customers = $this->Dsacustomers_model->getCustomers($id, $filter, $userType, $userType2, $date);
		...
		$Start_Date = date('Y-m-01'); 
				$End_Date  = date('Y-m-t');
				 $customers = $this->Admin_model->current_month2($Start_Date,$End_Date,$id);
		         $arr['customers']=$customers;
...
public function Lead_sourings()
  ...
  $searchQuery = " ";
  ## Total number of records without filtering
  $sel = $this->Admin_model->get_all_leadvisit($id,$userType);
  
 // $sel=$this->Admin_model->Get_Total_No_Customer();
  $totalRecords =$sel;
  ## Total number of records with filtering
  $sel=$this->Admin_model->get_all_customers_Admin_filters2($id,$searchValue,$Start_Date,$End_Date);
  $totalRecordwithFilter =$sel;
  
  ## Fetch records
  $sel=$this->Admin_model->get_all_customers_Admin_filter_paginations2($id,$searchValue,$columnName,$columnSortOrder,$row,$rowperpage,$Start_Date,$End_Date);
  $empRecords = $sel;
  $data = array();
        foreach($empRecords as $row)
        {
    ...
    if($this->session->userdata('USER_TYPE') == '' || $this->session->userdata('USER_TYPE') == 'CUSTOMER' || $this->session->userdata('SITE') != 'finserv'){
       ...
       // $customers = $this->Dsacustomers_model->getCustomers($id, $filter, $userType, $userType2, $date);
        ...
        $data['row']=$this->Customercrud_model->getProfileData($id);
		  $Curr_Date = date('Y-m-d');
				 $Today_Date  = date('Y-m-d');
				 $today = $this->Admin_model->current_month2($Curr_Date,$Today_Date,$id);
		         $arr['leadtoday']=$today;
		//$customers = $this->Admin_model->get_all_dsavisit();
        $data['username'] =$user_name;
        $arr['userType'] = $userType;
...
//------------------------------Function for combined excel data --------------------------//
				...
				$users=json_decode(json_encode($data_for_excel), true);
                 ...
                 foreach($users as $row)
				 {
					  ...
					  $this->excel->getActiveSheet(0)->setCellValue('B'.$count,$row['last_updated_date']);
                      ...
                      $this->excel->getActiveSheet(0)->setCellValue('H'.$count,$row['revisit_date']);
                ...
                $this->excel->setActiveSheetIndex(1)->setCellValue('F1', 'CUSTOMER INSTEREST');
				...
				$users1=json_decode(json_encode($data_for_excel1), true);
				  ...
				  foreach($users1 as $rows)
				 {
					  ...
					  $this->excel->getActiveSheet(1)->setCellValue('B'.$count,$rows['last_updated_date']);
					  ...
					  $this->excel->getActiveSheet(1)->setCellValue('G'.$count,$rows['next_action_date']);
				...
				$users2=json_decode(json_encode($data_for_excel2), true);
				  ...
				  foreach($users2 as $rows)
				 {
				      ...
				      $this->excel->getActiveSheet(2)->setCellValue('B'.$count,$rows['exist_date_of_meeting']);
...
//----------------------------------------For Current Month ------------------------------//	
		 ...
		 public Function save_excel()
				...
				$users=json_decode(json_encode($data_for_excel), true);
				 ...
				 foreach($users as $row)
				 {
					  ...
					  $this->excel->getActiveSheet()->setCellValue('B'.$count,$row['last_updated_date']);
                      ...
                      $this->excel->getActiveSheet()->setCellValue('H'.$count,$row['revisit_date']);
			...
			public Function Ntb_excel()
                ...
                $this->excel->setActiveSheetIndex(0)->setCellValue('F1', 'CUSTOMER INSTEREST');
				...
				$users=json_decode(json_encode($data_for_excel), true);
				 ...
				 foreach($users as $row)
				 {
                     ...
                     $this->excel->getActiveSheet(1)->setCellValue('A'.$count,$i);
					  $this->excel->getActiveSheet(1)->setCellValue('B'.$count,$row['last_updated_date']);
					  ...
					  $this->excel->getActiveSheet(1)->setCellValue('G'.$count,$row['next_action_date']);
		...
		public Function Existing_excel()
				...
				$users=json_decode(json_encode($data_for_excel), true);
				 ...
				 {
					$this->excel->getActiveSheet(0)->setCellValue('A'.$count,$i);
				    $this->excel->getActiveSheet(0)->setCellValue('B'.$count,$row['exist_date_of_meeting']);
